...
local function render(result)
  ...
  local lines = {
    ...
    "  worker: " .. vim.json.encode(result.worker),
    "  workerBound: " .. vim.json.encode(result.workerBound),
...
function M.run(raw)
  require("plurnk.workspace_context").resolve(function()
    ...
    if source == "" then
      client.send("worker.capabilities.get", {}, false, render)
      return
    end
    ...
    client.send("worker.capabilities.set", {
      policy = capabilities,
    }, false, render)
  end)
end
