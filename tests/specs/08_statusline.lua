...
local ok, err = pcall(function()
  ...
  H.assert_match(wb, "%(7/2%)%]", "the place carries the loop and its exact packet count")
  ...
  H.assert_truthy(wb:find("%(7/2%)%]") < wb:find("⌛︎") and wb:find("⌛︎") < wb:find("🤖 claude"), "winbar order is place → lifecycle → model")
  H.assert_truthy(not wb:match("P2"), "the packet count no longer stands alone; it rides the place")
  -- {§nvim-status-children}: the ant is the daemon's alive-children count, after the model
  H.assert_match(wb, "🤖 claude · 🐜2", "the daemon's alive-children count rides the winbar as the ant")
