...
`:AI/env` lists this Worker's environment — what its commands receive; `add <NAME> <value>` sets a variable for every command this Worker runs (the value verbatim), `disable` withholds a name, `discover [query]` lists the names you may set with their owning package. `:PlurnkEnv` is the native spelling.
